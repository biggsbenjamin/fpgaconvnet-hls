...
int main()
{
    ...
    // test inputs data
    static data_t test_in[SQUEEZE_BATCH_SIZE*SQUEEZE_ROWS*SQUEEZE_COLS*DIVIDE(SQUEEZE_CHANNELS,SQUEEZE_COARSE_IN)][SQUEEZE_COARSE_IN];
    static data_t test_out[SQUEEZE_BATCH_SIZE*SQUEEZE_ROWS*SQUEEZE_COLS*DIVIDE(SQUEEZE_CHANNELS,SQUEEZE_COARSE_OUT)][SQUEEZE_COARSE_OUT];
    ...
    for(int j=0;j<SQUEEZE_COARSE_OUT;j++) {
        err += checkStreamEqual<data_t>(out[j],out_valid[j], false);
    }
